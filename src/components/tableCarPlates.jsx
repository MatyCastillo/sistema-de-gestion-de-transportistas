...
export default function DataTable() {
  ...
  const [yearsToDropdown, setYearsToDropdown] = useState([]);
  const [yearSelect, setYearSelect] = useState("");
  var [rows, setRows] = useState([]);
  var [rows2, setRows2] = useState([]);
  var [onLoading, setLoading] = useState(false);
  ...
  useEffect(() => {
    (async () => {
      let res;
      res = await getYears();
      setYearsToDropdown(res);
    })();
  }, []);

  const yearFilter = async () => {
    const rowsByYear = await getProvsByYear(yearSelect);
    setRows(rowsByYear);
  };
