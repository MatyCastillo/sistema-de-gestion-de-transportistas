...
exports.updateProv = async (req, res) => {
  ...
  console.log("update prov:",req.params);
  const {
    prov_asoc,
    prov_dni,
    prov_nombre,
...
exports.actualizarImgPath = async (req, res) => {
  // const prov_id = req.body.prov_id;
  // const img_nombre = req.body.img_nombre;

  const { img_nombre, prov_id } = req.body;

  // Verifica que prov_id e img_nombre estén presentes en el cuerpo de la solicitud
  if (!prov_id || !img_nombre) {
    res.status(400).json({ 
      error: 'Se esperan campos prov_id e img_nombre en el cuerpo de la solicitud',
      message: JSON.stringify(req.body) + "_" + prov_id + "-" + img_nombre
    });
    return;
  }

  try {
    // Sentencia SQL para actualizar el img_path a NULL
    const sql = 'UPDATE images SET img_path = NULL WHERE prov_id = ? AND img_nombre = ?';

    // Ejecuta la consulta
    await new Promise((resolve, reject) => {
      mysqlConnection.query(sql, [prov_id, img_nombre], (error, results) => {
        if (error) {
          console.error('Error al actualizar img_path:', error);
          reject(error);
        } else {
          console.log('img_path actualizado correctamente');
          resolve();
        }
      });
    });

    res.status(200).json({ message: 'img_path actualizado correctamente' });
  } catch (error) {
    console.error('Error en la actualización de img_path:', error);
